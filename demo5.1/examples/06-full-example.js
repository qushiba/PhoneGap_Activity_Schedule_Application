...
function onThemeChange(me, mapsed) {
	...
	for (var i = 0; i < _snazzyMaps.length; i++) {
		...
		if (currTheme.name === themeName) {
			...
			mapsed.getGoogleMap().setOptions({
				styles: currTheme.theme
			});
			break;
		}
	}
...
function fullWindowExample() {
	...
	$.fn.mapsed({
		...
		mapOptions: {
			zoom: 15,
			center: new google.maps.LatLng(56.1506656, -3.9467269),
		...
		onSave: function(m, newPlace) {
			...
			if (newPlace) {
				...
				//m.showMsg("YOUR SAVE CODE HERE123",msg);



				//var datetime = window.opener.datetime.value;
				var url = "http://127.0.0.1/login/store1.php?userdata=" + newPlace.userData + "&lat=" + newPlace.lat + "&lng=" + newPlace.lng + "&location_name=" + newPlace.name + "&callback=?";
				$.getJSON(url, function(data) {
					if (data.result == "success") {
						m.showMsg("YOUR SAVE CODE HERE57567", msg);
						/*
						...
						if (queryString["username"] != null) {
							...
							//$("#lblData").html(data);
						} + "&username=" + encodeURIComponent(receive_username)
						 */


						var url1 = "caltest.html?userdata=" + encodeURIComponent(newPlace.userData);
		...
		onClose: function(mapsed) {
			...
			//var url2= "caltest.html?userdata=" + encodeURIComponent(newPlace.userData);
